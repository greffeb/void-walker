...
- **SCAN** (état=damaged, DC 9 PER)
  - réussite `flagSet=shuttle_searched`
...
- **EXAMINE** (état=open, auto)
  - réussite `flagSet=breach_examined`
...
- **atteignable** · `integrity=broken activity=inactive power=unpowered openness=open lock=unlocked` · via `examineResult`
  > Pieces recuperables eparpillees dans les debris : cablage, composants electroniques, outils. De quoi improviser des reparations.
- **atteignable** · `integrity=intact contents=empty` · via `descriptions`
...
- **USE sur `dr_okonkwo`** `flagSet=okonkwo_patched`
...
- **USE sur `dr_okonkwo`** `flagSet=escort_active`
...
- **atteignable** · `integrity=intact activity=inactive power=unpowered openness=open lock=unlocked` · via `descriptions`
...
- **atteignable** · `integrity=broken activity=inactive power=unpowered openness=open lock=unlocked` · via `examineResult`
  > Rack contenant un decoupeur plasma industriel. Puissant mais bruyant.
- **atteignable** · `integrity=intact contents=empty` · via `descriptions`
...
- **TALK** (état=intact, DC 8 CHA)
  - réussite `flagSet=okonkwo_found`
...
- **atteignable** · `integrity=broken activity=inactive power=unpowered openness=open lock=unlocked` · via `examineResult`
  > Terminal de recherche partiellement detruit. Les donnees recuperables montrent des resultats d'experiences genetiques.
- **atteignable** · `integrity=intact activity=active power=powered` · via `descriptions`
...
- **READ** (état=damaged, auto)
  - réussite `flagSet=project_hunter_read`
...
- **USE sur `creature_hunter`** `flagSet=creature_repelled_escalation`
...
- **USE sur `acoustic_walls`** `flagSet=creature_repelled_escalation`
...
- **atteignable** · `integrity=broken activity=inactive power=unpowered openness=open lock=unlocked` · via `examineResult`
  > Les parois de cette zone sont recouvertes de panneaux acoustiques. Un emetteur sonique fonctionnerait ici avec une efficacite maximale.
- **EXAMINE** (DC 10 INT)
  - réussite `flagSet=acoustic_info_received`
...
- **EXAMINE** (auto)
  - réussite `flagSet=acoustic_potential_noted`
...
- **atteignable** · `integrity=broken activity=inactive power=unpowered openness=open lock=unlocked` · via `examineResult`
  > Rack contenant des grenades flash et des generateurs de bruit. Utiles pour creer une diversion.
- **atteignable** · `integrity=intact contents=empty` · via `descriptions`
...
- **FORCE_OPEN** (état=damaged, objet=salvage_tool, auto)
  - réussite `newState=intact+open` `flagSet=blast_door_widened`
...
- **USE sur `creature_hunter`** `flagSet=creature_distracted`
...
> Vous voyez autour de vous une écoutille de navette, un point de piège acoustique ainsi qu'une porte de baie d'extraction.
...
- **MOVE_TO** (flag=escort_active, auto)
  - réussite `flagSet=both_in_shuttle`
...
- **MOVE_TO** (auto)
  - réussite `flagSet=okonkwo_abandoned`
...
- **USE/SACRIFICE** (flag=escort_active, DC 14 CHA)
  - réussite `flagSet=okonkwo_used_as_bait`
...
- **atteignable** · `integrity=broken activity=inactive power=unpowered openness=open lock=unlocked` · via `examineResult`
  > Point ideal pour un piege acoustique. La geometrie de la zone concentrerait les ondes sonores comme un amplificateur naturel.
- **atteignable** · `integrity=intact activity=active power=powered` · via `descriptions`
...
- **REPAIR** (état=damaged, DC 11 INT)
  - réussite `newState=intact+open` `flagSet=extraction_door_opened`
...
- **EXAMINE** (état=damaged, DC 10 PER)
  - réussite `flagSet=bay_door_bypass_found`
...
> Vous voyez autour de vous un cockpit de navette.
...
- **atteignable** · `activity=inactive power=unpowered integrity=broken openness=open lock=unlocked` · via `examineResult`
  > Le cockpit de la navette d'evacuation. Les systemes sont en ligne, les moteurs prets pour le decollage.
