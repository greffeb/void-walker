...
> À portée de main, vous remarquez un scanner portable ainsi qu'une trousse à outils standard.
...
- **atteignable** · `openness=open lock=unlocked integrity=broken activity=inactive power=unpowered` · via `descriptions`
  > Le mécanisme du sas est détruit. L'accès à la navette est bloqué physiquement.
- **atteignable** · `openness=closed lock=unlocked` · via `descriptions`
...
- **READ** (auto)
  - réussite `flagSet=manifest_read`
...
- **HACK** (DC 10 INT)
  - réussite `flagSet=manifest_hacked`
...
- **atteignable** · `activity=inactive power=powered` · via `descriptions`
...
- **USE sur `wall_safe`** `flagSet=safe_scanned`
...
- **USE sur `ai_core_node_a`** `flagSet=ai_scan_revealed`
...
- **USE sur `reactor_core`** `flagSet=reactor_sabotage_confirmed`
...
- **USE sur `ai_core_node_a`** `flagSet=node_a_exposed`
...
> Vous voyez autour de vous un terminal chiffré, un terminal de maintenance ainsi qu'un bloc-notes du directeur.
...
- **atteignable** · `lock=unlocked openness=open integrity=broken activity=inactive power=unpowered` · via `descriptions`
  > Terminal détruit. L'écran est fendu en étoile, les circuits grésillent. Les données sont inaccessibles par cette voie.
- **atteignable** · `lock=unlocked openness=closed activity=active power=powered` · via `descriptions`
...
- **USE** (état=locked, objet=encrypted_data_core, auto)
  - réussite `newState=unlocked+active` `flagSet=comms_unlocked`
...
- **READ** (état=damaged, auto)
  - réussite `flagSet=maintenance_logs_read`
...
- **HACK** (état=active, DC 12 INT)
  - réussite `flagSet=camera_evidence_found`
...
- **atteignable** · `integrity=intact contents=searched` · via `descriptions`
...
- **EXAMINE** (DC 10 PER)
  - réussite `flagSet=fraud_note_deciphered`
...
> Vous voyez autour de vous un terminal du directeur, un coffre-fort mural ainsi qu'un plan d'évacuation.
> À portée de main, vous remarquez des dossiers compromettants.
...
- **READ** (auto)
  - réussite `flagSet=revelation_read`
...
- **HACK** (DC 12 INT)
  - réussite `flagSet=classified_evidence_recovered`
...
- **HACK** (flag=revelation_read, DC 15 INT)
  - réussite `flagSet=vasquez_location_found`
...
- **EXAMINE** (flag=safe_scanned, auto)
  - réussite `flagSet=admin_badge_found`
...
- **READ** (auto)
  - réussite `flagSet=beacon_location_known`
...
- **EXAMINE** (DC 10 PER)
  - réussite `flagSet=trap_suspected`
...
- **USE sur `override_terminal`** `flagSet=override_admin_access`
...
- **USE sur `emergency_beacon`** `flagSet=evidence_transmitted`
...
> Vous voyez autour de vous un cœur du réacteur, un nœud IA primaire, un nœud IA secondaire ainsi qu'un terminal de neutralisation.
...
- **EXAMINE** (DC 10 PER)
  - réussite `flagSet=sabotage_evidence_reactor`
...
- **TALK** (état=active, DC 16 CHA)
  - réussite `flagSet=ai_talked_down`
...
> Vous voyez autour de vous une balise de détresse, un panneau de communications, un verrou final de l'IA ainsi qu'un écran de transmission balise.
...
- **ACTIVATE** (état=locked, flag=final_lock_opened, auto)
  - réussite `newState=unlocked+active` `flagSet=beacon_active`
...
- **ACTIVATE** (état=active, objet=incriminating_files, auto)
  - réussite `flagSet=evidence_transmitted`
...
- **HACK** (flag=manifest_hacked, DC 16 INT)
  - réussite `flagSet=comms_direct_access`
...
- **EXAMINE** (état=locked, DC 13 PER)
  - réussite `flagSet=lock_bypass_found`
...
> Vous voyez autour de vous un hublot de l'observatoire.
